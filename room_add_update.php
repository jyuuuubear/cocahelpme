...
if($url) { 
	...
	$return_url = "./viewer.php?mb_id=".$mb_id;
}
...
$in = sql_fetch("select * from {$g5['inventory_table']} inven, {$g5['item_table']} item where inven.in_id = '{$in_id}' and inven.it_id = item.it_id and inven.mb_id = '{$mb_id}'");
$it_id = $in['it_id'];
	...
	if (!preg_match("/\.(gif|jpg|png)$/i", $img)) {
		...
		if (file_exists($img_path)) {
			...
			if ($size[0] > $able_side[0] || $size[1] > $able_side[1]) {
				...
				$count = sql_fetch("select MAX(ro_id) as cnt from {$g5['room_table']} where mb_id = '{$mb_id}'");
				$count = $count['cnt']++;
				sql_query("insert into {$g5['room_table']} set ro_img='{$room_img}', ro_order='{$count}', mb_id = '{$mb_id}'");
